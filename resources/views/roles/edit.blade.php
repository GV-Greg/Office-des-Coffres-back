<x-app-layout>
    ...
    <x-content-card>
        <div class="grid grid-cols-1 justify-items-center">
            <form action="{{ route('role.edit', $role->id) }}" method="POST" class="w-full">
                @csrf
                        ...
                        <div class="grid grid-cols-4">
                            @foreach($permissions as $key => $permission)
                                <div class="form-check w-1/2">
                                    <input type="checkbox" id="{{ $permission->name }}" name="permission[]" value="{{ $permission->id }}" {{ in_array($permission->id, $rolePermissions) ? 'checked' : '' }}  class="form-check-input rounded text-blue-500">
